...
namespace OpenXMLPowerTools
{
    ...
    {
        private static MainDocumentPart mainDocumentPart = null;
        static readonly Regex cleanHtmlRegex = new Regex(@"<\?[^>]*>", RegexOptions.Compiled | RegexOptions.Singleline | RegexOptions.IgnoreCase);

        public Byte[] GenerateDocument(string TemplateDoc, XElement data)
        {
            FileInfo templateDoc = new FileInfo(TemplateDoc);
            if (!templateDoc.Exists)
            {
                throw new Exception(String.Format("Error, {0} does not exist.", TemplateDoc));
            }
            ...
            {
                throw new Exception("Some Internal Error", new Exception(String.Join(",", templateError.errors.ToArray())));
            }
        ...
        private static WmlDocument AssembleDocument(WmlDocument templateDoc, XElement data, out TemplateError templateError)
        {
            ...
            using (MemoryStream mem = new MemoryStream())
            {
                ...
                using (WordprocessingDocument wordDoc = WordprocessingDocument.Open(mem, true))
                {
                    ...
                    mainDocumentPart = wordDoc.MainDocumentPart;
        ...
        private static void ProcessTemplatePart(XElement data, TemplateError te, OpenXmlPart part)
        {
            ...
            ForceBlockLevelNestedElements(xDocRoot, te);

            xDoc.Elements().First().ReplaceWith(xDocRoot);

            xDoc.Descendants(W.r).Descendants(W.br)
                        .Where(x => (string)x.Attribute(W.type) == "page" &&
                                            x.Parent.Parent.Descendants(W.sectPr).Count() == 0 &&
                                            x.Parent.Parent.NextNode.ToString().Contains(@"<w:t>") == false)
                        .Select(x => x.Parent.Parent).Remove();
        ...
        private static XName[] s_NestedElementToForceBlock = new XName[]
        {
           W.p,
           W.tbl,
           W.table
        ...
        private static void ForceBlockLevelNestedElements(XElement xDoc, TemplateError te)
        {
            var paragraphs = xDoc.Descendants(W.p).ToList();

            foreach(var p in paragraphs)
            {
                var childMeta = p.Elements().Where(n => s_NestedElementToForceBlock.Contains(n.Name)).ToList();
                List<XElement> nelements = new List<XElement>();
                foreach (var e in childMeta)
                {
                    if (e.Name == W.p)
                    {
                        var meta = new XElement(e.Name,
                            p.Attributes(),
                            e.Elements(W.pPr),
                                e.Elements());
                        //p.AddAfterSelf(meta);
                ...
                if (p.Parent != null && nelements.Count > 0)
                {
                    p.AddAfterSelf(from e in nelements select e);
                }

                p.Elements().Where(n => s_NestedElementToForceBlock.Contains(n.Name)).Remove();
            }
        }
        ...
        static object ContentReplacementTransform(XNode node, XElement data, TemplateError templateError)
        {
            ...
            if (element != null)
            {
                if (element.Name == PA.Content)
                {
                    ...
                    var htmlString = (string)element.Attribute(PA.Html);
                    bool optional = (optionalString != null && optionalString.ToLower() == "true");
                    bool html = (htmlString != null && htmlString.ToLower() == "true");
                    ...
                    {
                        XElement p = null;
                        if (html && mainDocumentPart != null)
                        {
                            HtmlConverter converter = new HtmlConverter(mainDocumentPart);

                            var paragraphs = converter.Parse(cleanHtml(newValue));

                            for (int i = 0; i < paragraphs.Count; i++)
                            {
                                var xmlelement = paragraphs[i];

                                if (xmlelement.ChildElements.Count == 1)
                                {
                                    XElement xElement = XElement.Parse(xmlelement.InnerXml);
                                    p.Add(xElement);
                                }
                                else
                                {
                                    if (xmlelement.LocalName == "p")
                                    {
                                        foreach (var childelement in xmlelement.ChildElements)
                                        {
                                            XElement xElement = XElement.Parse(childelement.OuterXml);
                                            p.Add(xElement);
                                        }
                                    }
                                    else
                                    {
                                        XElement xElement = XElement.Parse(xmlelement.OuterXml);
                                        p.Add(xElement);
                                    }
                                }
                            }
                    ...
                    else
                    {
                        ...
                        if (html && mainDocumentPart != null)
                        {
                            ...
                            for (int i = 0; i < paragraphs.Count; i++)
                            {
                                ...
                                if (xmlelement is Table)
                                {
                                    // For some reason if you have an HTML table with a background color, it applies highlighting to text within the cells.
                                    // This works fine unless the background is a color (like orange) that doesn't appear in Word's highlighter options, in
                                    // which case it uses yellow instead, which just looks stupid.
                                    RemoveHighlighting(xmlelement);
                                }
                                ...
                                else
                                {
                                    if (xmlelement.LocalName == "p")
                                    {
                                        ...
                                        //          list.Add(xElement);
                                        ////          list.Add(new XElement(W.r,
                                        ////run.Elements().Where(e => e.Name != W.t), xElement));

                                        //      }
                                    }
                                    else if (xmlelement.LocalName == "tbl")
                                    {
                                        XElement xElement = XElement.Parse(xmlelement.OuterXml);
                                        //          list.Add(new XElement(W.r,
                                        //run.Elements().Where(e => e.Name != W.t),
                ...
                var ele = new XElement(element.Name,
                    element.Attributes(),
        ...
        private static XElement CreateRunErrorMessage(string errorMessage, TemplateError templateError)
        {
            ...
            templateError.errors.Add(errorMessage);
        ...
        {
            return cleanHtmlRegex.Replace(html, string.Empty);
        }

        private static void RemoveHighlighting(OpenXmlCompositeElement part)
        {
            for (int i = part.ChildElements.Count - 1; i >= 0; --i)
            {
                var child = part.ChildElements[i];
                if (child is Highlight)
                    part.RemoveChild(child);
                if (child is OpenXmlCompositeElement)
                    RemoveHighlighting((OpenXmlCompositeElement)child);
            }
        }
    }
}